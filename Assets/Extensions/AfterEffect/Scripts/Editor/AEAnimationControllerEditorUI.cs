...
public class AEAnimationControllerEditorUI : Editor {
		...
		foreach(AEClipTemplate tpl in controller.clips) {
			...
			if(tpl.IsEditMode) {
				...
				if(EditorGUI.EndChangeCheck()) {
					controller.SetClipNameUI(name, tpl.animUI);
				}
			}
	...
	private void SearchLayers() {
		...
		foreach(Transform tr in controller.transform) {
			...
			//}
            AfterEffectAnimationUi animUI = tr.GetComponent<AfterEffectAnimationUi>();
            if (animUI != null)
            {
                controller.RegisterClipUI(animUI);
            }
