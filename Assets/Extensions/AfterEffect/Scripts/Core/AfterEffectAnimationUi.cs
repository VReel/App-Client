...
[ExecuteInEditMode]
[System.Serializable]
public class AfterEffectAnimationUi : MonoBehaviour {
	...
	public GameObject spritesHolder {
		get {
			if(_spritesHolder == null) {
				...
				_spritesHolder = AEResourceManager.CreateAnimationRootUI();
				_spritesHolder.name = "SpritesHolder";
				_spritesHolder.transform.SetParent(transform);
	...
	public float GetLayerGlobalZ(float index, AESpriteUI sp) {
	...
	private void InitSprites() {
		...
		foreach(AELayerTemplate layer in _animationData.composition.layers) {
            ...
            sprite.transform.SetSiblingIndex(10 - layer.index);
            Debug.Log("SiblingIndex = " + sprite.transform.GetSiblingIndex() + " " + sprite.name);
	...
	protected virtual AEFootageUI CreateFootage() {
		return AEResourceManager.CreateFootageUI ();
	}

	protected virtual AEFootageUI CreateSpriteFootage() {
		return AEResourceManager.CreateSpriteFootageUI();
	}

	protected virtual AEFootageUI CreateUIFootage() {
		return AEResourceManager.CreateUIFootageUI ();
	}
